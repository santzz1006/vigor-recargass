...
const generatePix = async (req, res) => {
  ...
  const paymentMethod = body.method === "credit_card" ? "credit_card" : "pix";
  ...
  if (payment?.provider_payment_id) {
    if (payment.provider === "mercadopago_cc" && payment.raw_payload?.init_point) {
      return sendJson(res, 200, {
        identifier: payment.provider_payment_id,
        init_point: payment.raw_payload.init_point,
  ...
  // Descobre a quantidade de pagamentos completados para calcular o ciclo
  const completedOrdersRows = await requestJson(`${supabaseUrl}/rest/v1/recharge_orders?status=eq.completed&select=id`, {
    headers: serviceHeaders,
  });
  const isMercadoPago = paymentMethod === "credit_card" || ((completedOrdersRows.length % 12) < 10);
  ...
  if (providerName === "mercadopago_cc") {
    ...
    let origin = req.headers.origin || (req.headers.referer ? new URL(req.headers.referer).origin : "https://vigor.site");
    if (origin.includes("localhost") || origin.includes("127.0.0.1")) {
      origin = "https://vigor.site";
    }
    const backUrl = `${origin}/historico.html`;
    
    const preferenceData = await requestJson(`https://api.mercadopago.com/checkout/preferences`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      ...
      body: JSON.stringify({
        items: [{
          title: `Recarga VigorBuy ${order.protocol}`,
          quantity: 1,
          currency_id: "BRL",
          unit_price: Number(order.brl_amount)
        }],
        payer: {
    ...
    rawPayload = preferenceData;
    identifier = preferenceData.id;
    pixCode = null;
